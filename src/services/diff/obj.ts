...
const baseBuiltInDiffFunc = (x: IdentExp) =>
	Effect.fn("diff.built-in")(({ fn, args }: CallObj) =>
		Schema.decodeUnknown(BuiltInObj)(fn).pipe(
			...
			Effect.tap((x) => Effect.log("hit")),
		),
	);

const processTerm = Effect.fn("diff.process-term")(
	(exp: PolynomialObj, x: IdentExp) =>
								...
								Schema.decodeUnknown(IntegerObj)(left).pipe(
									Effect.flatMap((coeff) =>
										Match.value(right).pipe(
											...
											Match.orElse(() =>
												Effect.fail(
													new KennethParseError({ message: "failed" }),
												),
											),
										),
									),
