...
describe('parser', () => {
	...
	test('boolean literal expression', () => {
		...
		const program = (expStmt: ExpStmt) =>
			Effect.gen(function* () {
				const boolExp = yield* Schema.decodeUnknown(BoolExp)(expStmt.expression)
	...
	test('nested if  expression', () => {
		...
		const program = (expStmt: ExpStmt) =>
			Effect.gen(function* () {
				...
				expect(innerFirstConsequence.tokenLiteral()).toBe('return')
				yield* testLiteralExpression(innerFirstConsequence.value, 10)

				expect(ifExp.alternative).toBeUndefined()

				expect(consequence2.tokenLiteral()).toBe('return')
	...
	test('function literal parsing', () => {
		...
		const program = (expStmt: ExpStmt) =>
			Effect.gen(function* () {
				const funcExp = yield* Schema.decodeUnknown(FuncExp)(expStmt.expression)
	...
	test('call expression parsing', () => {
		...
		const program = (expStmt: ExpStmt) =>
			Effect.gen(function* () {
				const callExp = yield* Schema.decodeUnknown(CallExp)(expStmt.expression)
	...
	test('identifier expression', () => {
		...
		const program = (expStmt: ExpStmt) =>
			Effect.gen(function* () {
				...
				expect(identExp.tokenLiteral()).toBe('foobar')
			})
